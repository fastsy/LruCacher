...
public class MainActivity extends AppCompatActivity {
    ...
    public final static String[] imgs = {"https://lh6.googleusercontent.com/-55osAWw3x0Q/URquUtcFr5I/AAAAAAAAAbs/rWlj1RUKrYI/s160-c/A%252520Photographer.jpg",
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        MyAdapter adapter = new MyAdapter(ls);
        ls.setAdapter(adapter);
    }


    private List<DownLoadAsynTask> downLoadTasks = new ArrayList<>();

    class MyAdapter extends BaseAdapter implements AbsListView.OnScrollListener{

        public MyAdapter(ListView ls){
        ...
        public void onScrollStateChanged(AbsListView view, int scrollState) {
            if (scrollState == SCROLL_STATE_IDLE){
                //滑动停止时加载图片
                loadBitmap(firstItem,visibleItem);
            }else{
                cancelTask();
            }
        }
        ...
        private void cancelTask(){
            for (DownLoadAsynTask task : downLoadTasks){
                task.cancel(false);
            }
        }
    }

    private void loadBitmap(int firstItem , int visibleItem){
        for (int i = firstItem ; i< firstItem + visibleItem ; i++){
            String url = imgs[i];
            Bitmap bitmap = lrucacher.getBitmapFromMemory(url);
            if (bitmap == null){
                DownLoadAsynTask task = new DownLoadAsynTask();
                task.execute(imgs[i]);
                downLoadTasks.add(task);
            }else{
                ImageView iv = (ImageView) ls.findViewWithTag(imgs[i]);
                iv.setImageBitmap(bitmap);
            }
     ...
     * 下载bitmap栈
     */
    class DownLoadAsynTask extends AsyncTask<String,Integer,Bitmap> {
        ...
        protected Bitmap doInBackground(String... params) {
            ...
            if (bitmap != null){
                lrucacher.putBitmapToMemory(params[0],bitmap);
            }
            return bitmap;
        }
        ...
        protected void onPostExecute(Bitmap bitmap) {
            ImageView iv = (ImageView) ls.findViewWithTag(imgUrl);
